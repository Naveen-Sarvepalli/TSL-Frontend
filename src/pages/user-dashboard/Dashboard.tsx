...
function buildSlaPdf(d: SlaWizardData, completedAt: string | null): Blob {
  ...
  const lines: string[] = [
    ...
    ...(d.provider?.regNumber || d.providerReg ? [`  Reg No.     : ${v(d.provider?.regNumber || d.providerReg)}`] : []),
    ...(d.provider?.email || d.providerEmail ? [`  Email       : ${v(d.provider?.email || d.providerEmail)}`] : []),
    '',
    'Customer',
    `  Name        : ${v(d.customer?.legalName || d.customer?.fullNames || d.customerName)}`,
    ...(d.customer?.regNumber || d.customerReg ? [`  Reg No.     : ${v(d.customer?.regNumber || d.customerReg)}`] : []),
    ...(d.customer?.email || d.customerEmail ? [`  Email       : ${v(d.customer?.email || d.customerEmail)}`] : []),
    '',
    '─────────────────────────────────────────',
    '2. AGREEMENT DETAILS',
    '─────────────────────────────────────────',
